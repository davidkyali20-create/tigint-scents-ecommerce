...
export default function StarterPackWizard({ onAddPackageToCart }: StarterPackWizardProps) {
  ...
  return (
    <div className="bg-white border border-[#ecd1cc] rounded-2xl p-6 lg:p-8 shadow-md relative overflow-hidden" id="starter-pack-wizard">
      <div className="absolute top-0 right-0 w-64 h-64 bg-gradient-to-br from-rose-100/35 to-transparent rounded-full blur-3xl pointer-events-none"></div>
      
      {/* Package Value Proposition Header */}
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 border-b border-[#f5e3e0] pb-6 mb-6">
        <div>
          <span className="inline-flex items-center gap-1 bg-[#fff0ef] text-[#6e1329] text-xs font-bold px-3 py-1 rounded-full border border-[#ecd1cc] mb-3 uppercase tracking-wider font-display">
            🎯 Starter Package — KES 10,000
          </span>
          <h3 className="text-2xl font-bold font-display text-[#6e1329] tracking-tight">
            Launch Your Own Perfume Business Today
          </h3>
          <p className="text-zinc-600 text-sm mt-1 max-w-xl">
            Everything you need to double your investment in one week. Includes <strong className="text-[#6e1329]">10 x 30ml Pure Grade 1 Oils</strong> + <strong className="text-[#6e1329]">2 Dozen (24 pcs) Empty 6ml Roll-on Bottles</strong>.
          </p>
        </div>
        <div className="text-right flex flex-col items-start lg:items-end">
          <p className="text-xs text-zinc-400">Guaranteed Return</p>
          <p className="text-3xl font-extrabold text-[#6e1329] font-mono">KES 10,000</p>
          <div className="flex gap-1.5 mt-1 text-[11px] text-[#6e1329] items-center bg-[#fff0ef] px-2.5 py-1 rounded-full border border-[#ecd1cc]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <form onSubmit={handleGenerate} className="lg:col-span-5 flex flex-col justify-between space-y-4">
          <div>
            <h4 className="text-sm font-bold text-[#6e1329] mb-3 font-display flex items-center gap-1.5">
              <Sparkles size={16} className="text-[#cca43b]" /> AI-Powered Scent Curator
            </h4>
              ...
              {DEMOGRAPHIC_PRESETS.map((p) => (
                <button
                  ...
                  className={`p-2.5 rounded-xl text-left border text-[11px] transition cursor-pointer ${
                    demographic === p.text
                      ? "border-[#6e1329] bg-[#fff0ef] text-[#6e1329] font-semibold"
                      : "border-zinc-200 bg-zinc-50/50 text-zinc-600 hover:border-[#ecd1cc] hover:bg-[#fffbfb]"
                  }`}
                >
                  <span className="font-bold block text-[#6e1329]">{p.label}</span>
                </button>
              ))}
            </div>
            ...
            <textarea
              ...
              className="w-full bg-white border border-[#ecd1cc] rounded-lg p-2.5 text-xs text-zinc-800 focus:outline-none focus:ring-2 focus:ring-[#6e1329] mb-4 h-16"
              required
            />
            ...
            <input
              ...
              className="w-full bg-white border border-[#ecd1cc] rounded-lg p-2.5 text-xs text-zinc-800 focus:outline-none focus:ring-2 focus:ring-[#6e1329] mb-4"
            />
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3 px-4 bg-[#cca43b] hover:bg-[#b8912e] disabled:bg-zinc-200 text-black font-extrabold rounded-lg text-xs flex items-center justify-center gap-2 shadow-sm transition duration-200 uppercase tracking-wider font-display cursor-pointer"
          >
            {loading ? (
              <>
                <Loader2 size={16} className="animate-spin text-black" /> Designing Portfolio with Gemini...
              </>
            ) : (
              <>
                <Sparkles size={16} className="fill-current text-black" /> Auto-Select 10 Best Scents
              </>
            )}
          </button>
        </form>
        ...
        {/* Output Panel */}
        <div className="lg:col-span-7 bg-[#fffbfb] border border-[#f5e3e0] rounded-2xl p-5 flex flex-col justify-between min-h-[300px]">
          {loading ? (
            <div className="flex-1 flex flex-col items-center justify-center text-center p-6">
              <Loader2 size={36} className="animate-spin text-[#cca43b] mb-3" />
              <p className="text-sm font-bold text-[#6e1329]">Analyzing Perfume Chemistry & Nairobi Trends</p>
            ...
            <div className="space-y-4 flex-1 flex flex-col justify-between">
              <div>
                <div className="flex items-center justify-between border-b border-[#f5e3e0] pb-3">
                  <h5 className="text-xs font-bold text-[#6e1329] uppercase tracking-widest font-display">
                    Recommended 10-Portion Scent Composition
                  </h5>
                  <span className="text-[10px] bg-rose-100 text-[#6e1329] font-bold px-2 py-0.5 rounded-full">
                    Optimized Portfolio
                  </span>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
                  {aiResult.recommendedScents.map((s, idx) => (
                    <div key={idx} className="bg-white border border-[#f5e3e0] rounded-xl p-3 relative shadow-sm">
                      <div className="absolute top-2.5 right-2.5 bg-[#fff0ef] text-[#6e1329] font-mono text-xs font-bold px-1.5 py-0.5 rounded border border-[#ecd1cc]">
                        {s.suggestedQty} portions
                      </div>
                      <p className="font-bold text-xs text-[#6e1329] pr-16">{s.scentName}</p>
                      <p className="text-[10px] text-zinc-600 mt-1 italic">{s.reason}</p>
                    </div>
                  ))}
                </div>

                {/* Profit math banner */}
                <div className="bg-[#fff0ef] border border-[#ecd1cc] rounded-xl p-3.5 mt-4">
                  <p className="text-[10px] font-bold text-[#6e1329] uppercase tracking-wider">Projected Business Profit Margin</p>
                ...
                <div className="bg-white border border-zinc-200 rounded-xl p-3.5 mt-3">
                  <p className="text-[10px] font-bold text-[#6e1329] uppercase tracking-wider">CBD Reseller Action Strategy</p>
              ...
              <div className="pt-4 border-t border-[#f5e3e0]">
                <button
                  type="button"
                  ...
                  className={`w-full py-3 rounded-lg font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-2 transition duration-200 cursor-pointer ${
                    isAdded 
                      ...
                      : "bg-[#cca43b] hover:bg-[#b8912e] text-black shadow-md hover:shadow-[#cca43b]/20"
                  }`}
                >
                  {isAdded ? (
                    <>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center p-6">
              <div className="w-12 h-12 rounded-full bg-rose-50 border border-[#ecd1cc] flex items-center justify-center text-[#cca43b] mb-3">
                <Info size={20} />
              </div>
              <p className="text-sm font-bold text-[#6e1329]">Portfolios Ready for Creation</p>
              ...
              {/* Promo Banner inside */}
              <div className="mt-4 bg-white border border-[#ecd1cc] rounded-xl p-3 flex items-center gap-2.5 text-left max-w-sm shadow-sm">
                <span className="p-1 rounded bg-[#fff0ef] text-[#6e1329]">
                  <Flame size={14} />
                </span>
                <p className="text-[10px] text-zinc-600">
                  <strong className="text-[#6e1329]">Tip:</strong> Select <strong>"Campus & College Students"</strong> for sweet, fruity, vanilla blends with rapid stock turnarounds.
                </p>
              </div>
            </div>
          )}
